...
// --- Admin Panel & Form Logic (unchanged) ---
...
uploadForm.addEventListener('submit', async (e) => {
    ...
    if (!imgbbUploadUrl) {
        alert("Ошибка: Ключ API для загрузки изображений не загружен. Попробуйте обновить страницу.");
        return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    database.ref('api-imgbb').once('value').then((snapshot) => {
        const apiKey = snapshot.val();
        if (apiKey) {
            imgbbUploadUrl = `https://api.imgbb.com/1/upload?key=${apiKey}`;
        } else {
            console.error("IMGBB API Key not found in the database.");
            // Disable the upload button if the key is not found
            const uploadButton = document.querySelector('#upload-form button[type="submit"]');
            if(uploadButton) {
                uploadButton.disabled = true;
                uploadButton.textContent = "Ошибка загрузки";
            }
        }
    }).catch(error => {
